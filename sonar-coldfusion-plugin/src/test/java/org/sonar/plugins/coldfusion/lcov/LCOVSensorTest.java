...
public class LCOVSensorTest {
  ...
  private final File baseDir = new File("src/test/resources/org/sonar/plugins/coldfusion/jstestdriver/sensortests/main");
  ...
  public void init() {
    ...
    settings.setProperty(ColdFusionPlugin.LCOV_REPORT_PATH, "jsTestDriver.conf-coverage.dat");
    sensor = new LCOVSensor(new ColdFusion(settings));
  ...
  public void test_should_execute() {
    ...
    settings.setProperty(ColdFusionPlugin.LCOV_REPORT_PATH, "");
    assertThat(sensor.shouldExecuteOnProject(project)).isFalse();
  }
  ...
  public void testFileInJsTestDriverCoverageReport() {
    when(fileSystem.mainFiles(ColdFusion.KEY)).thenReturn(Arrays.asList(InputFileUtils.create(baseDir, "Person.js")));

    when(fileSystem.resolvePath("jsTestDriver.conf-coverage.dat"))
      .thenReturn(new File("src/test/resources/org/sonar/plugins/coldfusion/jstestdriver/jsTestDriver.conf-coverage.dat"));
    when(fileSystem.resolvePath("Person.js"))
      .thenReturn(new File("src/test/resources/org/sonar/plugins/coldfusion/jstestdriver/sensortests/main/Person.js"));
    when(fileSystem.resolvePath("PersonTest.js"))
      .thenReturn(new File("src/test/resources/org/sonar/plugins/coldfusion/jstestdriver/sensortests/test/PersonTest.js"));
  ...
  public void testFileNotInJsTestDriverCoverageReport() {
    ...
    when(fileSystem.mainFiles(ColdFusion.KEY)).thenReturn(Arrays.asList(inputFile));
